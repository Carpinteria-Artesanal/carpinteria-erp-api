...
const _checkId = async id => {
  const deliveryOrderExist = await BudgetModel.exists({ _id: id });
...
const validateProductIndex = async ({
  ...
  const { products } = await BudgetModel.findOne({ _id: id });
...
const hasDate = async ({ params: { id } }) => {
  const deliveryOrder = await BudgetModel.findOne({ _id: id });
